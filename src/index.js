...
fetch(`http://localhost:3000/users/${saveBtn.id}`)
.then(response => response.json())
...
function deleteOrLikeAdvice (event) {
  ...
  if (event.target.innerText == "♻️") {
    ...
    fetch(`http://localhost:3000/advices/${splitAdviceId}`,{ method: "DELETE"})
  } else {

    fetch(`http://localhost:3000/advices/${splitAdviceId}`, {
        method: "PATCH",
